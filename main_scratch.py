...
if __name__ == "__main__":
    ...
    params = json.load(open(f'train_configs/scratch/{args.train_config}.json'))
    ...
    for experiment_idx in range(num_folds):
        ...
        X_phate_test= [X_phate[i] for i in [0]+test_idx]
        X_phate_conditional_test = [X_phate_conditional[i] for i in [0]+test_idx]
        try:
            Spatial_test = [Spatial[i] for i in [0]+test_idx]
            Celltype_list_test = [Celltype_list[i] for i in [0]+test_idx]
        except:
            Spatial_test = []
            Celltype_list_test = []
        ...
        weighted_wasserstein_data = {'clf_model': multi_class_clf, 'gt_labels': Celltype_list, 'label_encoder': label_encoder}
