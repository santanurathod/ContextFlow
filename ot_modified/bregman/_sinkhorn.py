    ...
    **Choosing a Sinkhorn solver**
    ...
    if method.lower() == 'sinkhorn':
        ...
        return sinkhorn_epsilon_scaling(a, b, M, reg, numItermax=numItermax,
                                        ...
                                        verbose=verbose, log=log, warn=warn,
                                        **kwargs)
    elif method.lower() == 'sinkhorn_relative_entropy':
        Q_prior = kwargs.get('Q_prior')
        return sinkhorn_relative_entropy(a, b, M, reg, numItermax=numItermax,
                                        stopThr=stopThr, warmstart=warmstart,
                                        verbose=verbose, log=log, warn=warn,
                                        Q_prior=Q_prior)
    else:
...
def sinkhorn_relative_entropy(a, b, M, reg, numItermax=1000, stopThr=1e-9,
                   verbose=False, log=False, warn=True, warmstart=None, Q_prior=None, **kwargs):
    r"""
    Solve the entropic regularization optimal transport problem and return the OT matrix
    ...
    if Q_prior is None:
        raise ValueError("Q_prior is required for sinkhorn_relative_entropy")
